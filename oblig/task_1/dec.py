...
def main():
    if len(sys.argv) != 5:
        ...
        # Read the ciphertext, key, and IV from files and convert to bits
        ciphertext = bytes_to_bits(read_file(ciphertext_file))
        ...
        for block in ciphertext_blocks:
            # decrypt the cipher block with the key
            plaintext = decrypt(block, key)
            # xor the result of decryption with the plaintext_block
            plaintext = XOR(plaintext, plaintext_block)
            # save the plaintext as the next plaintext block to be XORed with next block
            plaintext_block = block
            # concatenate the plaintext with the previous results
            final_plaintext += ''.join(str(e) for e in plaintext)
